...
const Watchlisttable = () => {
    ...
    // TODO: this code is duplicated, make one shared version?
    ...
    const fetchWatchlist = () => {
        ...
        fetch('https://stock-backend-dot-rice-comp-539-spring-2022.uk.r.appspot.com/user/watchlist/get', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
            ...
            .then(data => {
                console.log("user watchlist successful")
                console.log(data);
                setWatchStocks(data);

                return Promise.all([
                    fetchAllChangePercents(data),
                    fetchAllCurrentPrices(data),
                    fetchAllStockNames(data)
                ]);
            })
    ...
    const handleWatchlistClickDashboard = (symbol) => {
            ...
            <div className="header-row">
                ...
                <div className="watchlist-cell">
                    <div className="watchlist-icon">
                        <div className="subheading-2 description"></div>
                    </div>
                    <div className="divider"></div>
                </div>
            </div>
            ...
            <div className="scrollable-body">
                {watchStocks.map((stockSymbol) =>
                    <div key={stockSymbol} className="row" onClick={() => handleRowClick(stockSymbol, stockNames[stockSymbol])}
                        ...
                        <div className="watchlist-cell">
                            <div className="watchlist-icon" style={{justifyContent: "center"}}>
                                <div className="tooltip-parent">
                                    <button className="watchlisttable-button" onClick={() => handleWatchlistClickDashboard(stockSymbol)}>
                                        <img src={removewatchlist} alt="watchlist button icon" />
                                        <span className="tooltip-text-watchlist">Remove</span>
                                    </button>
                                </div>
                            </div>
                            <div className="divider"></div>
                ...
                {Object.keys(watchStocks).length === 0 && (
                    <div className="row">
                        <div className="empty-table" style={{ textAlign: 'center', width: '100%' }}>
                            <div className="subheading-2-table">Your watchlist is empty.</div>
                        </div>
                    </div>
                )}
            </div>
